"""
批量调用 DeepSeek 对话 / 推理模型的脚本。

密钥管理方式：
- 请在项目根目录的 config/api_keys.json 中为 "deepseek" 配置密钥；
- 本脚本通过公共工具函数读取该密钥，不在代码中硬编码任何密钥。

本版本仅保留进度估算，不再计算 token 数量或价格。
"""
...
def _get_deepseek_secret() -> str:
    """获取 DeepSeek 服务的密钥。"""
    data = _load_keys()
    try:
        return data["deepseek"]["api_key"]
    except KeyError as e:
        raise KeyError("在 config/api_keys.json 中未找到 'deepseek.api_key' 配置") from e
...
def _parse_temperature_from_filename(filename: str) -> float:
    """从文件名中提取温度值，例如 input_v3_t0.7.jsonl → 0.7。"""
...
def estimate_file(input_file: str, output_file: str, input_files_dict) -> None:
    """仅估算还有多少条数据待处理（不再计算 token 用量和价格）。"""
    input_filename = os.path.basename(input_file)

    # 根据文件名设定模型
    if "v3" in input_filename:
...
def process_file(input_file: str, output_file: str, input_files_dict) -> None:
    ...
    temperature = _parse_temperature_from_filename(input_filename)
    ...
    for i, item in enumerate(processed_data):
        ...
        try:
            ...
            if model_id == "deepseek-chat":
                ...
                tag1 = response.choices[0].logprobs.content[0].top_logprobs[0].token
                prob1 = response.choices[0].logprobs.content[0].top_logprobs[0].logprob
                tag2 = response.choices[0].logprobs.content[0].top_logprobs[1].token
                prob2 = response.choices[0].logprobs.content[0].top_logprobs[1].logprob
                tag3 = response.choices[0].logprobs.content[0].top_logprobs[2].token
                prob3 = response.choices[0].logprobs.content[0].top_logprobs[2].logprob
                assistant_content = (
                    f"{assistant}***prob({tag1})={prob1},"
                    f"prob({tag2})={prob2}, prob({tag3})={prob3}"
                )
            else:
                assistant = response.choices[0].message.content
            ...
            if error_count == 1:
                print(f"\n{error_msg} (r{retry_count}e{error_count})")
            else:
                print(f"{error_msg} (r{retry_count}e{error_count})")
            if error_count > 15:
                if retry_count < 3:
                    time.sleep(600)
                    retry_count += 1
